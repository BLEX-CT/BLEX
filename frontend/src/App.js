...
const _D={bg:"#080808",surface:"#111",card:"#141414",cardHover:"#1a1a1a",border:"#222",text:"#f0f0f0",muted:"#666",sub:"#333",chip:"#1c1c1c",input:"#141414",inputBorder:"#2a2a2a",success:"#22c55e",error:"#ef4444",overlay:"rgba(0,0,0,0.86)",glow:"rgba(255,255,255,0.03)"};
const THEMES={
  dark: {..._D,accent:"#e8e8e8",accentTxt:"#080808",nav:"rgba(8,8,8,0.93)",navBorder:"#1a1a1a"},
...
function injectCSS(){
  ...
  s.textContent=`
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .btn-t{transition:opacity .15s,transform .15s,background .2s,color .2s}
    .btn-t:hover{opacity:.84;transform:scale(1.03)}.btn-t:active{transform:scale(.97)}
    .card-wrap{transition:transform .28s ease,box-shadow .28s ease}
    .card-wrap:hover{transform:translateY(-6px)}
    .img-ov{opacity:0;transition:opacity .25s}.card-wrap:hover .img-ov{opacity:1}
    .hero-title{font-size:clamp(56px,12vw,128px);font-weight:900;letter-spacing:14px;background:linear-gradient(135deg,#fff 0%,#aaa 50%,#fff 100%);background-size:200% 200%;-webkit-background-clip:text;-webkit-text-fill-color:transparent;animation:shimmer 4s linear infinite}
...
export default function App() {
  ...
  const addToCart=p=>{setCart(prev=>{const ex=prev.find(i=>i.id===p.id);return ex?prev.map(i=>i.id===p.id?{...i,qty:i.qty+1}:i):[...prev,{...p,qty:1}];});setCartOpen(true);addToast(`${p.name.substring(0,24)} added`,"success");};
  ...
  const filtered=useMemo(()=>sp.filter(p=>{const s=search.toLowerCase();return(category==="all"||p.category===category)&&(p.name.toLowerCase().includes(s)||(p.description||"").toLowerCase().includes(s));}),[sp,search,category]);
  ...
  if(maintPreview||(maintenance?.enabled&&!maintBypass&&!adminAuth)) return(<div style={{position:"fixed",inset:0,background:"#030303",color:"#fff",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",zIndex:9999,overflow:"hidden",fontFamily:"'Segoe UI',system-ui,sans-serif"}}>
    ...
    <div style={{position:"relative",zIndex:2,textAlign:"center",maxWidth:"480px",width:"100%",padding:"0 20px"}}>
      ...
      <div style={{display:"flex",gap:"22px",justifyContent:"center"}}>{[["Twitter/X","𝕏"],["Instagram","◎"],["LinkedIn","in"]].map(([n,ic])=><a key={n} href="#" onClick={e=>e.preventDefault()} style={{color:"#2a2a2a",fontSize:"24px",textDecoration:"none",fontWeight:"700",transition:"color .2s"}} onMouseEnter={e=>e.currentTarget.style.color="#fff"} onMouseLeave={e=>e.currentTarget.style.color="#2a2a2a"} title={n}>{ic}</a>)}</div>
    </div>
  </div>);
  return (
    ...
    {/* NAVBAR */}
    <nav style={{background:c.nav,borderBottom:`1px solid ${c.navBorder}`,padding:"0 18px",height:"58px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:"8px",position:"sticky",top:0,zIndex:100,backdropFilter:"blur(18px)"}}>
      ...
      <div style={{flex:1,maxWidth:"340px",position:"relative"}}>
        ...
        <input value={searchRaw} onChange={e=>{setSearchRaw(e.target.value);setView("store");}} placeholder={t.search} style={{...inp(false),borderRadius:"20px",[isRtl?"paddingRight":"paddingLeft"]:"33px",paddingTop:"7px",paddingBottom:"7px",fontSize:"13px"}}/>
      </div>
    ...
    {view==="store"&&<div>
      <div style={{position:"relative",overflow:"hidden",minHeight:"500px",background:heroImage?`url(${heroImage}) center/cover`:theme==="dark"?"linear-gradient(145deg,#080808 0%,#0f0f0f 50%,#111 100%)":"linear-gradient(145deg,#f0f0f0 0%,#e8e8e8 100%)",display:"flex",flexDirection:"column",justifyContent:"center"}}>
        ...
        <div style={{position:"relative",zIndex:2,textAlign:"center",padding:"64px 24px 48px"}}>
          ...
          <div className="fu" style={{animationDelay:".2s"}}><p style={{color:c.muted,fontSize:"14px",marginTop:"6px",marginBottom:"30px",letterSpacing:"1px"}}>{t.electronics} · {t.accessories} · {t.clothing}</p></div>
          ...
          <div className="fu" style={{animationDelay:".42s",display:"flex",justifyContent:"center",gap:"44px",marginTop:"44px",flexWrap:"wrap"}}>
            {[t.stat1,t.stat2,t.stat3].map((s,i)=><div key={i} style={{textAlign:"center"}}><p style={{fontWeight:"800",fontSize:"17px"}}>{s.split(" ")[0]}</p><p style={{color:c.muted,fontSize:"11px",marginTop:"2px"}}>{s.split(" ").slice(1).join(" ")}</p></div>)}
          </div>
        </div>
          ...
          :getLocalOrders().filter(o=>o.customerEmail===user.email).map((o,i)=>(
            <div key={i} style={{background:c.card,borderRadius:"12px",border:`1px solid ${c.border}`,padding:"14px",marginBottom:"10px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"6px"}}>
                ...
                <span style={{fontWeight:"700",fontSize:"14px"}}>{fmt(o.total)}</span>
              </div>
